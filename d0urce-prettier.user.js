...
// Helper stuffs for dealing with event styled items
...
listen(({ data, socket, event }) => {
  ...
  if (typeof payload !== "object") return console.log(data);
    ...
    traverse(payload, function(item) {
        ...
        let count = 0;
        let testTotal = 0.0;
        if(item && item.player_inventory) {
          traverse(item, (invItem) => {
            if(invItem && invItem.mint) {
              count++;
              const index = rarities.indexOf(String(invItem.rarity).toLowerCase());
              const type = invItem.type === 'firewall' ? 'router' : invItem.type;
              if(nonCosmetics.includes(type)) {
                  const level = invItem.upgradeLevel;
                  const effects = statsToEffect(invItem.stats);
                  const standardGrade = type != 'gpu' ? getItemGrade(type, level, index, effects, true) : effects["Idle Crypto Mining"] - stats.gpu_term[index] * (level - 1);
                  const percentile = dPM(standardGrade, index, type);

                  let price = dPS(percentile, level, index);
                  console.warn(`${invItem.name} (${invItem.type}: ${price})`)
                  testTotal += price.split("~").length > 1
                    ? (parseFloat(price.split("~")[0]) + parseFloat(price.split("~")[1])) / 2
                    : parseFloat(price.split("+")[0])
              } else {
                  const mint = invItem.mint;
                  const premium = invItem.premium;
                  const price = dCPS(type, index, mint, premium);
